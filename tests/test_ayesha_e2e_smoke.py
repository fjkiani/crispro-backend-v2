...
class TestAyeshaE2ESmoke:
    ...
    async def test_ayesha_trials_clinical_validation(self):
        ...
        async with httpx.AsyncClient(base_url="http://127.0.0.1:8000", timeout=30.0) as client:
            ...
            assert ca125["current_value"] == 2842.0
            
            # Forecast should have cycle 3 and cycle 6 expectations
            forecast = ca125["forecast"]
            assert "cycle3_expected_drop" in forecast
            assert "cycle6_expected_drop" in forecast
            assert "target_remission" in forecast
            ...
            for test in ngs["checklist"]:
                if "ctDNA" in test.get("test_name", ""):
                    ctdna_found = True
                    assert test["recommended"] == True
                    assert test["turnaround_time_days"] <= 10
                if "HRD" in test.get("test_name", "") or "MyChoice" in test.get("test_name", ""):
                    hrd_found = True
                    assert test["recommended"] == True
            
            assert ctdna_found, "ctDNA test not found in NGS checklist"
            assert hrd_found, "HRD test not found in NGS checklist"
