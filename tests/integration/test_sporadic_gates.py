...
def test_tmb_high_boost():
    ...
    # Expected:  1.35x boost for TMB-high (TMB ≥20)
    ...
    print(f"✅ TMB boost test passed: {efficacy_score:.2f} (0.60 → 0.81)")
...
def test_msi_high_boost():
    ...
    print(f"✅ MSI boost test passed: {efficacy_score:.2f} (0.60 → 0.81)")
...
def test_tmb_msi_double_boost():
    ...
    # Current implementation checks TMB ≥20 first and applies 1.35x; MSI is not multiplied.
    assert efficacy_score == pytest.approx(0.81, abs=0.01), f"Expected 0.81 (0.60 * 1.35), got {efficacy_score}"
    assert confidence == pytest.approx(0.70, abs=0.01), "Confidence should not change"
    ...
    print(f"✅ Double boost test passed: {efficacy_score:.2f} (0.60 → 0.81, TMB priority)")
