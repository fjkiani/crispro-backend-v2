#!/usr/bin/env python3
"""
⚔️ FIND BEST TRIALS FOR AYESHA - LIVE ASTRADB SEARCH ⚔️

Uses LIVE vector search against AstraDB (not static 50 candidates).
Fully configurable filters via FilterConfig.
...
# Output directory
OUTPUT_DIR = Path(__file__).resolve().parent.parent.parent / ".cursor" / "ayesha" / "zo_live_astradb_dossiers"
...
async def search_astradb_for_ayesha(ayesha, top_k: int = 200, min_score: float = 0.5):
    ...
    Args:
        ...
        top_k: Number of trials to retrieve (default: 200)
    ...
    Looking for recruiting trials in USA (NY, NJ, CT region preferred).
...
async def main():
    print("⚔️ FINDING BEST TRIALS FOR AYESHA - LIVE ASTRADB SEARCH ⚔️\n")
    ...
    # Option 1: NYC Metro only (default)
    config = FilterConfig()
    config.ALLOWED_STATES = {'NY', 'NJ', 'CT'}
    print(f"   Allowed states: {config.ALLOWED_STATES}")
    
    # Option 2: Expanded to include more Northeast states
    # Uncomment to enable:
    # config = get_expanded_states_config({'NY', 'NJ', 'CT', 'PA', 'MA', 'RI', 'MD', 'DE'})
    # print(f"   Allowed states (EXPANDED): {config.ALLOWED_STATES}")
    
    # Option 3: Allow more treatment lines
    # config.LINE_SCORE_WEIGHTS['maintenance'] = 0.9  # Boost maintenance
    # config.LINE_SCORE_WEIGHTS['recurrent'] = 0.5    # Allow recurrent
    ...
    print(f"   Treatment line weights: {config.LINE_SCORE_WEIGHTS}")
