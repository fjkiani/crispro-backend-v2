"""
⚔️ PRODUCTION - Concern A: Candidate Discovery Agent (D1-D3)
...
- D1: Build profile → search queries (agent framework)
...
try:
    ...
    logger.warning("⚠️ Trial intelligence pipeline not available")
...
try:
    ...
    logger.warning("⚠️ Autonomous trial agent not available")
...
def _extract_profile_field(profile: Dict[str, Any], field_paths: List[str], default: str = '') -> str:
    """
    ⚔️ FIX: Extract profile field handling both nested and flat formats.
...
def build_profile_search_queries(
    ...
    """
    ⚔️ D1: Build profile → search queries
    ...
    if not AUTONOMOUS_AGENT_AVAILABLE:
        ...
        # ⚔️ FIX: Handle both nested and flat profile formats
        ...
        # ⚔️ FIX: Ensure we always generate at least one query, even if fields are empty
        ...
        # ⚔️ FIX: Fallback to generic cancer search if no fields available
        if not queries:
            queries.append("cancer clinical trial")
            logger.warning("⚠️ No profile fields available - using generic cancer search")
...
def fetch_candidates_from_sqlite(
    ...
    """
    ⚔️ D2: Fetch candidates from local store (SQLite first)
    ...
    try:
        ...
        # ⚔️ FIX: Build WHERE clause from filters (work even if queries are empty)
        where_clauses = []
        params = []
        ...
        if not disease:
            ...
            disease = 'cancer'
            logger.warning("⚠️ No disease provided - using generic 'cancer' search")
        ...
        if treatment_line == 0:  # Frontline
            where_clauses.append("""
                (lower(coalesce(summary,'')) NOT LIKE '%recurrent%'
                 AND lower(coalesce(inclusion_criteria,'')) NOT LIKE '%prior%chemotherapy%')
            """)
        ...
        logger.info(f"✅ Fetched {len(candidates)} candidates from SQLite")
        return candidates, provenance
    
    except Exception as e:
        logger.error(f"❌ SQLite fetch failed: {e}")
        return [], {
            "source": "sqlite",
            "status": "failed",
            "reason": str(e),
...
async def fetch_candidates_from_ctgov(
    ...
    """
    ⚔️ D2: Fallback to CT.gov search (bounded)
    ...
    logger.warning("⚠️ CT.gov fallback not implemented yet")
...
def enforce_scope_boundaries(
    ...
    logger.info(f"✅ Enforced scope boundaries: {candidates_returned} candidates (screened {candidates_screened})")
...
async def discover_candidates(
    ...
    """
    ⚔️ Main candidate discovery pipeline (D1-D3)
    ...
    """
    logger.info("⚔️ Starting candidate discovery pipeline (D1-D3)")
    
    # D1: Build profile → search queries
    logger.info("📋 D1: Building profile → search queries")
    try:
    ...
    # ⚔️ FIX: Extract filters from profile (handle both nested and flat formats)
    filters = {
        'disease': _extract_profile_field(
            patient_profile,
            ['disease.primary_diagnosis', 'disease', 'primary_diagnosis']
    ...
    logger.info("🔍 D2: Fetching candidates from SQLite")
    ...
    if len(candidates) < min_candidates and not prefer_sqlite:
        logger.info("🔍 D2: SQLite insufficient, falling back to CT.gov")
    ...
    logger.info("⚙️ D3: Enforcing scope boundaries")
  ...
  python discovery_agent.py --profile-file profile.json
    ...
    if args.profile == "ayesha":
        try:
            ...
            logger.error("❌ Ayesha profile not available")
            sys.exit(1)
    elif args.profile_file:
        patient_profile = json.loads(Path(args.profile_file).read_text())
    else:
        logger.error("❌ Must specify --profile or --profile-file")
        sys.exit(1)
