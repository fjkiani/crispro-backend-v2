...
class BaseResistanceDetector(ABC):
    ...
    def _emit_signal_detected(self, signal_data: ResistanceSignalData):
        ...
        if self.event_emitter:
            try:
                # Support both event dispatcher and simple dict-based emitters
                if hasattr(self.event_emitter, 'emit_signal_detected'):
                    detector_id = self.__class__.__name__
                    self.event_emitter.emit_signal_detected(signal_data, detector_id)
                else:
    ...
    def _emit_signal_absent(self, signal_type: str, reason: str):
        ...
        if self.event_emitter:
            try:
                # Support both event dispatcher and simple dict-based emitters
                if hasattr(self.event_emitter, 'emit_signal_absent'):
                    detector_id = self.__class__.__name__
                    self.event_emitter.emit_signal_absent(signal_type, reason, detector_id)
                else:
                    # Fallback to simple emit
